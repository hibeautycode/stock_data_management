import sys, os, time, threading, re, datetime
...
# 凹波形参数：下降阈值 min_descend_rate
	...
	def analyse_spill_data( self, list_descend_rate_interval, process_queue, filter_singular_rate = 0.2 ):
		...
		LOG( '{4} pid-{0} finish analyzing. descend_rate_interval:[ {1:.3f} {2:.3f} ] growth_rate:{3:.3f}'.format( os.getpid(), list_descend_rate_interval[ 0 ], list_descend_rate_interval[ 1 ], self.growth_rate, time.strftime('%Y-%m-%d %H:%M:%S',time.localtime( time.time() ) ) ) ) 
		...
		for descend_rate in arange( self.min_descend_rate, self.max_descend_rate, self.descend_rate_interval_len ):
			for growth_rate in arange( self.min_growth_rate, self.max_growth_rate, self.growth_rate_interval_len ):
				...
				if len( list_process ) == 10:
					...
					num_minutes = 0
					LOG( '{0} resting for {1} minutes…'.format( time.strftime('%Y-%m-%d %H:%M:%S',time.localtime( time.time() ) ), num_minutes ) )
			...
			for index in df_value_stock.index:
				...
				try:
					...
					if float( df_realtime_quotes[ 'price' ] ) >= ( float( df_value_stock.loc[ index ][ 'buy_price' ] ) * 0.99 ) :
						LOG( 'opportunity-{0}	cur price:{1:02.2f}		buy price:{2:02.2f}		expect earn rate:{3:02.2f}'\
							.format( code, float( df_realtime_quotes[ 'price' ] ), \
						...
						content_notify += '--opportunity-{0}\tcur price:{1:.2f}\tbuy price:{2:.2f}\texpect earn rate:{3:.2f}\n'\
							.format( code, float( df_realtime_quotes[ 'price' ] ), \
									float( df_value_stock.loc[ index ][ 'buy_price' ] ), \
									float( df_value_stock.loc[ index ][ 'expect_earn_rate' ] ), \
									float( df_value_stock.loc[ index ][ 'min_earn_rate' ] ) )	
...
if __name__ == '__main__':
	...
	if int( cur_time.split( ':' )[ 0 ] ) < 18:
	# 18点之前，采用前一天的分析结果
		file_date = datetime.date.today() - datetime.timedelta( days = 1 )
	else:
		file_date = datetime.date.today()	

	file_date = file_date.strftime( '%Y_%m_%d' )
